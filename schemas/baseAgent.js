...
export const baseAgentExtendedResponseSchema = {
  ...
  properties: {
    choice: {
      type: "string",
      enum: ["response", "code", "terminalCommand", "plan"],
      description: "Type of response being provided. 'response' for conversational, 'code' for code generation, 'terminalCommand' for terminal execution, 'plan' for creating a step-by-step execution plan"
    },
    ...
    // Plan fields (used when choice = "plan")
    steps: {
      type: "array",
      description: "List of steps in the execution plan (when choice='plan')",
      items: { type: "string" }
    },
    planExplanation: {
      type: "string",
      description: "Overall explanation of the plan and strategy (when choice='plan')"
    },
  ...
  required: ["choice", "response", "questionsForUser", "questions", "missingContext", "code", "language", "codeExplanation", "terminalCommand", "commandReasoning", "requiresApproval", "steps", "planExplanation", "tool", "continue"],
